...
<p>Complete planes at X = ±111 mm are saved every two solver steps, normally 50 µs apart. Full fields are checkpointed every half-hour of wall time using lossless native gzip compression. The solver records pressure and velocity probes, field bounds and ambient flux each step. A verified signal handler writes a restart checkpoint at the requested stop. New diagnostic downloads use lossless gzip; MP4 compression retains every plotted flow state.</p>
{steady_note}
{flowing_note}
