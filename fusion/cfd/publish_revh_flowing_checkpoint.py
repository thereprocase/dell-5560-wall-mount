...
def main():
    ...
    rank_note=(' The run began with four workers and was checkpointed onto eight after a short field-equivalence and throughput comparison; physical time and native previous-step fields were preserved. <a href="../core-scaling/">Measured CPU scaling and checkpoint checks</a>.' if worker_count==8 else '')
    opening_html=''
    opening=PAGE/'opening-flow-0.001546810911/opening-flow.json'
    if opening.is_file():
        measurement=json.loads(opening.read_text())
        front=measurement['results']['front_mouth']['net_flow_L_s']
        upward=measurement['results']['upward_channel']['net_flow_L_s']
        opening_html=f'''<h2>Where the front-opening flow goes</h2>
<figure><a href="opening-flow-0.001546810911/opening-flow.png"><img src="opening-flow-0.001546810911/opening-flow.png" alt="Measured normal velocity across the front mouth and upward passage over the central 280 mm" loading="lazy"></a>
<figcaption>Saved transient field at {measurement['transient_time_s']*1000:.4f} ms: net upward flow {upward:.2f} L/s versus net outward front flow {front:.2f} L/s across these defined cuts. The same dominant upward direction appears in the starting steady iterate. These open cuts do not form a closed device flow budget. <a href="opening-flow-0.001546810911/opening-flow.json">Integration definitions and values</a>.</figcaption></figure>'''
