...
def main():
    ...
    p.add_argument('--source-case',default='revh_steady_09')
    p.add_argument('--steady-iteration',type=int,default=400)
    p.add_argument('--physical-time',type=float,help='Seconds after the flowing-field initialization; never a SIMPLE iteration')
    args=p.parse_args();args.output.mkdir(exist_ok=False)
    warm=args.physical_time is not None
    assert not warm or (args.physical_time>=0 and args.source_case!='revh_steady_09')
    source_time=args.physical_time if warm else args.steady_iteration
    source_label=(f'Transient {args.physical_time*1000:.4f} ms after flowing-field initialization' if warm else
                  f'Steady iteration {args.steady_iteration}, unconverged')
    ...
    results={};polys={};hashes={};definitions={};stored_times={}
    for name in ['front_mouth','upward_channel']:
        ...
        stored_time=poly.GetFieldData().GetArray('TimeValue')
        if stored_time:
            stored_times[name]=stored_time.GetTuple1(0)
            assert abs(stored_times[name]-source_time)<=1e-6*max(abs(source_time),1e-9),'Source time label does not match the VTP metadata'
    ...
    report={'created_utc':datetime.now(timezone.utc).isoformat(),'source_case':args.source_case,
        'steady_iteration':None if warm else args.steady_iteration,'transient_time_s':args.physical_time,
        'time_kind':'physical_seconds_after_flowing_initialization' if warm else 'SIMPLE_iteration',
        'vtp_time_metadata':stored_times,'converged':False,
    ...
    fig.text(.09,.878,source_label+'. Gap axes are expanded.',fontsize=13,color='#17686b')
