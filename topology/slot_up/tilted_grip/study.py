...
H=2.; NX,NY,NZ=25,48,6


# 230.3 mm laptop; top 80 mm reserved for grasping. 20 mm laptop in
# a 22 mm slot, nominal 1 mm side clearances. Rotate slot/seat together.
import math
LAPTOP_HEIGHT=230.3
GRIP_LENGTH=80.
from scipy.optimize import brentq
ANGLE=brentq(lambda a:20+math.cos(a)+(LAPTOP_HEIGHT-GRIP_LENGTH)*math.sin(a)-32,0,.3)
def transform(x,y,z):
    c,s=math.cos(ANGLE),math.sin(ANGLE)
    xr=20+c*(x-20)+s*(y-6);yr=6-s*(x-20)+c*(y-6)
    blend=min(1.,max(0.,(x-10)/8))
    return (x+blend*(xr-x),y+blend*(yr-y),z)
def local_node(n):
    i=(n-1)%(NX+1);j=((n-1)//(NX+1))%(NY+1);k=(n-1)//((NX+1)*(NY+1))
    return (H*i,H*j,H*k)
    ...
    for k in range(NZ):
        for j in range(NY):
            for i in range(NX):
                ...
                if 20<x<42 and y>6 and z>4:continue # laptop insertion volume
                e=en(i,j,k)
                elements[e]=[nid(i,j,k),nid(i+1,j,k),nid(i+1,j+1,k),nid(i,j+1,k),nid(i,j,k+1),nid(i+1,j,k+1),nid(i+1,j+1,k+1),nid(i,j+1,k+1)]
                if (x<10 and (6<y<24 or 72<y<96)) or (16<x<50 and y<6) or (18<x<20 and y>6) or (42<x<44 and y>6) or (22<x<40 and 74<y<86 and z<4):keep.add(e)
    return {n:transform(*p) for n,p in nodes.items()},elements,keep
...
def deck(path,active=None,scales=None):
    ...
    selection={n:local_node(n) for n in used}
    groups={
      'SUPPORT':[n for n in used if selection[n][0]==0 and (10<=selection[n][1]<=20 or 80<=selection[n][1]<=90)],
      'SEAT':[n for n in used if selection[n][1]==6 and 22<=selection[n][0]<=40 and 6<=selection[n][2]<=10],
      'FRONT':[n for n in used if selection[n][0]==42 and 80<=selection[n][1]<=90 and 6<=selection[n][2]<=10],
      'SIDE':[n for n in used if selection[n][2]==4 and 24<=selection[n][0]<=38 and 76<=selection[n][1]<=84]}
    assert len(groups['SEAT'])==30 and len(groups['FRONT'])==18
