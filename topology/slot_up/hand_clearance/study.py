    ...
    for k in range(NZ):
        for j in range(NY):
            for i in range(NX):
                ...
                if 20<x<40 and y>6 and z>4:continue # laptop insertion volume
                e=en(i,j,k)
                elements[e]=[nid(i,j,k),nid(i+1,j,k),nid(i+1,j+1,k),nid(i,j+1,k),nid(i,j,k+1),nid(i+1,j,k+1),nid(i+1,j+1,k+1),nid(i,j+1,k+1)]
                if (x<10 and (6<y<24 or 72<y<96)) or (16<x<48 and y<6) or (18<x<20 and y>6) or (40<x<42 and y>6) or (22<x<38 and 74<y<86 and z<4):keep.add(e)
...
def deck(path,active=None,scales=None):
    ...
    groups={
      ...
      'SEAT':[n for n in used if nodes[n][1]==6 and 22<=nodes[n][0]<=38 and 6<=nodes[n][2]<=10],
      'FRONT':[n for n in used if nodes[n][0]==40 and 80<=nodes[n][1]<=90 and 6<=nodes[n][2]<=10],
      'SIDE':[n for n in used if nodes[n][2]==4 and 24<=nodes[n][0]<=36 and 76<=nodes[n][1]<=84]}
