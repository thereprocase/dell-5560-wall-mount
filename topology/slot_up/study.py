    ...
    for k in range(NZ):
        for j in range(NY):
            for i in range(NX):
                ...
                if 28<x<48 and y>6 and z>4:continue # laptop insertion volume
                e=en(i,j,k)
                elements[e]=[nid(i,j,k),nid(i+1,j,k),nid(i+1,j+1,k),nid(i,j+1,k),nid(i,j,k+1),nid(i+1,j,k+1),nid(i+1,j+1,k+1),nid(i,j+1,k+1)]
                if (x<10 and (6<y<24 or 72<y<96)) or (24<x<56 and y<6) or (26<x<28 and y>6) or (48<x<50 and y>6) or (30<x<46 and 74<y<86 and z<4):keep.add(e)
...
def deck(path,active=None,scales=None):
    ...
    groups={
      ...
      'SEAT':[n for n in used if nodes[n][1]==6 and 30<=nodes[n][0]<=46 and 6<=nodes[n][2]<=10],
      'FRONT':[n for n in used if nodes[n][0]==48 and 80<=nodes[n][1]<=90 and 6<=nodes[n][2]<=10],
      'SIDE':[n for n in used if nodes[n][2]==4 and 32<=nodes[n][0]<=44 and 76<=nodes[n][1]<=84]}
...
def run():
    runtime=ROOT.parent/'runtime';work=ROOT/'work';work.mkdir(exist_ok=True)
