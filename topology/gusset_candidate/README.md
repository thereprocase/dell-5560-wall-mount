# Gusseted fanless mount — current review build

This is the current **cloud-topo** design, with six curved gussets per bracket: two mirrored PETG brackets with a curved side frame, large openings, a bottom seat and a slot open at the top. It reconstructs the earlier topology study as editable CAD. **It is a fit/print prototype, not a strength-qualified installation release.**

![Current CAD](results/curved_bracket.jpg)

The gussets broaden the contact-tab roots and wall-pad junctions, then taper inward toward the top of the print. They add **7.51 g per pair**. At the finer mesh, predicted movement drops **38% under the downward case, 18% under the outward grab and 40% under the side load**. Peak grab stress remains essentially unchanged, so these results demonstrate improved screened stiffness rather than a proven strength rating.
...
The FreeCAD document uses stock Sketcher, Part, Spreadsheet and Link objects. It reopens without a custom Python feature proxy. Six fastener-tool sketches are fully constrained; the twenty-two frame/profile/loft sketches have editable cubic control points and are **not fully constrained**.
...
4. Run `check_layers.py` after Orca finishes, then `check_structure.py 1.8` and `check_structure.py 1.2` in order, then `compare.py`. `render_review.py` renders the exported STL with VTK; headless Linux may need EGL/Mesa.
